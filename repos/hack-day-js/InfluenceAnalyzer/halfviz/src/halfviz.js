...
//    sys = arbor.ParticleSystem(2600, 512, 0.5);
    sys = arbor.ParticleSystem(500, 250, 0.5);
	...
	// TODO: use HTML 5 File IO instead
      getDoc:function(e){
      ...
      newDoc:function(){
		...
		updateGraph:function(e){
			...
			$.each(network.nodes, function(nname, ndata){
				if (ndata.viewInflow === true) {
					network.edges = new Graph().getAllInflowEdges(nname, edges);
				} else if (ndata.viewOutflow === true) {
					network.edges = new Graph().getAllOutflowEdges(nname, edges);
				}
			});

			// save the updated data to local storage
			$.jStorage.set('network', network);
